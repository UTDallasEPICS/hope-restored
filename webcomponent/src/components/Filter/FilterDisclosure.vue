<template>
  <Disclosure v-slot="{ open }">
    ...
    <transition
        ...
        enter-from-class="transform translate-y-[-10px] opacity-0 max-h-0"
        enter-to-class="transform translate-y-0 opacity-100"
        leave-active-class="transition-all duration-500 overflow-hidden"
        leave-from-class="transform translate-y-0 opacity-100"
        leave-to-class="transform translate-y-[-10px] opacity-0 max-h-0"
        @before-enter="setAnimationStatus(true)"
        @after-leave="setAnimationStatus(false)"
    >
    <!-- disclosurepanel -->
      <DisclosurePanel :class="`space-y-2 mt-2 ${maxHStyling} overflow-y-auto`">
        <div
          v-if="group.items.length>0" v-for="item in group.items"
...
const maxDisclosurePanelHeight = 60
const maxHStyling :string = "max-h-"+maxDisclosurePanelHeight;
</script>
